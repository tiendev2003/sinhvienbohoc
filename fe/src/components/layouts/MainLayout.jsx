...
const MainLayout = ({ isAuthenticated }) => {
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      {/* Sidebar */}
      <div className={`bg-gray-800 text-white ${isSidebarOpen ? 'w-72' : 'w-20'} space-y-6 py-4 px-2 absolute inset-y-0 left-0 transform transition-all duration-300 ease-in-out overflow-y-auto z-10 md:relative`}>
        <div className="flex items-center justify-between px-4 border-b border-gray-700 pb-4">
          {isSidebarOpen ? (
            ...
            <button onClick={toggleSidebar} className="p-2 mx-auto rounded-md hover:bg-gray-700">
        ...
        <nav className="space-y-3">
                  ...
                  {visibleItems.map((item, index) => {
                    ...
                    return (
                      <Link 
                        key={index} 
                        to={item.path} 
                        className={`flex items-center ${isSidebarOpen ? 'gap-3' : 'justify-center'} py-2 px-4 rounded-lg transition duration-200 ${
                          isActive 
          ...
          <div className={`px-${isSidebarOpen ? '6' : '2'} pt-4 border-t border-gray-700`}>
            <button 
              onClick={handleLogout} 
              className={`flex items-center ${isSidebarOpen ? 'gap-3' : 'justify-center'} w-full text-left py-2 px-4 rounded-lg transition duration-200 text-gray-300 hover:bg-gray-700 hover:text-white`}
