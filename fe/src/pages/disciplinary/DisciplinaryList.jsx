...
import { deleteDisciplinaryRecord, fetchDisciplinaryRecords } from '../../services/api';
...
const DisciplinaryList = () => {
  ...
  const filteredRecords = records.filter(record => {
    ...
    const matchesSeverity = filterSeverity === 'all' || record.severity_level === filterSeverity;
  ...
  const columns = [
    ...
    { 
      ...
      cell: (row) => {
        const severityColor = 
          row.severity_level === 'minor' ? 'bg-yellow-100 text-yellow-800' :
          row.severity_level === 'moderate' ? 'bg-orange-100 text-orange-800' :
          'bg-red-100 text-red-800';
        
        const severityText = 
          row.severity_level === 'minor' ? 'Nhẹ' :
          row.severity_level === 'moderate' ? 'Trung bình' :
          'Nghiêm trọng';
  ...
  if (loading) return <div className="flex justify-center p-8">Loading disciplinary records...</div>;
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Danh Sách Vi Phạm Kỷ Luật</h1>
